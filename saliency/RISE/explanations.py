...
class RISE(nn.Module):
    def __init__(self, input_size, gpu_batch=100, N=6000, s=2, p1=0.1):
    ...
    def generate_masks(self, N, s, p1, savepath='masks.npy'):
        ...
        for i in tqdm(range(N), desc='Generating filters'):
            ...
            resized = resize(grid[i], up_size, order=1, mode='reflect',anti_aliasing=False)
            cropped = resized[x:x + self.input_size[0], y:y + self.input_size[1], z:z + self.input_size[2]]
            self.masks[i,:,:] = cropped
    ...
    def forward(self, x, node, weighted_average):
        print(self.masks.shape, x.shape)
        N = self.N
        H, W = x.size()
        # _, _, H, W = x.size()
        ...
        for i in range(0, N, self.gpu_batch):
            curr_img = stack[i:min(i + self.gpu_batch, N)]
