...
class RISE(nn.Module):
    def __init__(self, model, input_size, gpu_batch=100, N=6000, s=8, p1=0.1):
    ...
    def forward(self, x, node, weighted_average):
        N = self.N
        _, _, H, W = x.size()
        ...
        for i in range(0, N, self.gpu_batch):
            curr_img = self.model(stack[i:min(i + self.gpu_batch, N)])
            output_sub = HardTreeSupLoss.get_output_sub(curr_img, node, weighted_average)
            # turn to distribution
            output_sub = F.softmax(output_sub)
            p.append(output_sub)
        p = torch.cat(p)
        # Number of classes
        CL = p.size(1)
    ...
    def explain_instance(self, x, node, weighted_average, topk=1):
        saliency = self.forward(x, node, weighted_average)
        output_sub = HardTreeSupLoss.get_output_sub(self.model(x), node, weighted_average)
        output_sub = F.softmax(output_sub)
        p, c = torch.topk(output_sub, k=topk)
        p, c = p[0], c[0]
