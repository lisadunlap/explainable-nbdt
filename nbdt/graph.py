...
def get_path_to_node(G, node):
    ...
    return list(nx.shortest_path(G, source=get_root(G), target=node))
...
def build_self_induced_graph(wnids, checkpoint, linkage='ward', affinity='euclidean',
        branching_factor=2, ignore_labels=[]):
    # TODO: get weight and bias matrices here, send to build_full
    ...
    G, nodes = build_full(centers, bias, naive, classes=classes, wnids=wnids)
