...
__all__ = names = ('CIFAR10IncludeLabels',
                   ...
                   'MiniPlaces', 'AnimalsWithAttributes2')
...
class AnimalsWithAttributes2(Dataset): 
    def __init__(self, root, transform, train=True, download=False, shuffle=False, **kwargs):
        # load in the data, we ignore test case, only train/val
        self.root = os.path.join(root, 'awa2')
        labels_fname = 'trainclasses.txt' if train else 'testclasses.txt'
        photos_path = os.path.join(self.root, 'JPEGImages')
        labels_path = os.path.join(self.root, labels_fname)
        ...
        for image_folder in os.listdir(self.photos_path):
            if image_folder in self.classes:
